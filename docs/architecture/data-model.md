...
| `pass_id`          | Identifies the recitation in progress. **Derived, not random:** computed from `user_id`, `practice_id`, `practice_version` and `pass_ordinal`                    |
...
**A recitation counts once, on any number of devices.** The count event's id comes from `pass_id`, which is derived rather than random, so two devices finishing the same pass offline produce the **same** event id. The server keeps the first and ignores the second, which matches the merged `pass_ordinal`: one pass finished, one recitation counted. If combining marks from two devices completes every step, the pass finishes on whichever device notices first, under that same derived id, so it still counts once. Before sign-in the owner key is the device's own local profile, so two devices used as guests can't match ids; each one's recitation is kept when they later sign in, which is right, since each device was chanted on separately.
