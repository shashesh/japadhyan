...
| `idempotency_key`      | Empty for ordinary events. For a finished namavali recitation it is derived from `user_id`, `practice_id`, `practice_version` and `pass_ordinal`, **worked out at the moment the recitation finishes**. Unique per user on the server, so the same recitation can never be stored twice |
...
**A recitation counts once, on any number of devices.** The count event keeps its own UUIDv7 id, and carries an `idempotency_key` derived from the owner, the practice, its version and the pass number. Two devices finishing the same pass offline produce the same key, and the server keeps only the first, which matches the merged `pass_ordinal`: one pass finished, one recitation counted. The key is worked out **when the recitation finishes**, from whoever owns the data at that moment, so signing in part-way through a recitation needs no special handling. If combining marks from two devices completes every step, the pass finishes on whichever device notices first, under that same key, so it still counts once. Before sign-in the owner is the device's own local profile, so two devices used as guests can't match keys; each one's recitation is kept when they later sign in, which is right, since each device was chanted on separately.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Never changed once sealed        | `count_events`                                                                                         | Server inserts and ignores an id it already has, and likewise a repeated `idempotency_key` for the same user. Row-level security blocks updates and deletes                                                                       |
...
- `CountEvent` gains `local_day`, `tz_offset_min`, `steps_per_repetition` and `idempotency_key`. `dailyTotals` groups by `local_day` instead of converting `created_at`.
