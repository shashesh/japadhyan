...
- **Closed by default.** Supabase grants every new table and function in `public` to `anon` and `authenticated`; the first migration turns those defaults off, so each object is granted explicitly or not at all. Nothing for `anon`. `authenticated` may read its own rows; insert sessions and count events; update a session's `ended_at` only, once (a column grant, and a trigger that raises `23514` if it changes again); and never write positions directly.
