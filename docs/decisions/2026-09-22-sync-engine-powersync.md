...
- **Data hosted is the one cost that grows without bound.** It tracks cumulative registered devotees × lifetime history, not daily users, so churned accounts keep costing money: ~$227/month at 100k users in year one, ~$700 by year three, breaching the plan's default cap. The fix is ours to design — keep recent events in the publication and move older ones to an archive table outside it, syncing a daily rollup for the heatmap. Totals stay derived from events; only the synced window narrows. PowerSync's own time-windowing needs a cron job that rewrites rows, which would violate our sealed-events invariant, so we should not use it.
...
2. Two devices chant the same practice offline, reconnect, and `totalCount` is exact while two `practice_positions` rows converge with their bitsets unioned.
