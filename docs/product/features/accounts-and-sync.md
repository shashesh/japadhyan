...
- **Import ignores who owned the file.** Every imported record is re-keyed to whoever is using the app now: the local profile, or the signed-in account. Generated ids are kept; ids [derived from the owner](../../architecture/data-model.md#ids-for-rows-that-are-unique-per-devotee) (saved practices, deity defaults, positions) are recomputed for the new owner, and a recomputed id that matches an existing row is combined with it. Both are deterministic, which is what makes a repeat import harmless. Nothing imported can ever be uploaded under someone else's account.
