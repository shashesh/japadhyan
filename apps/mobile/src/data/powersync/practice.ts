 ...
 * clock. Each runs inside the caller's write transaction. The dev screen and
 * the headless devices in tools/sync-lab both use them.
 *
...
import { correctedNow, requireDeviceState, type DeviceState } from './deviceState';
...
/**
 * Where new ids get their randomness. `uuidv7` defaults to Web Crypto, which
 * Node and browsers have and a native runtime doesn't, so the app passes
 * Expo's.
 */
...
/** Runs `action` in one write transaction, with the device's state as it is inside it. */
export function writePractice<T>(
  db: CommonPowerSyncDatabase,
  { nowMs, randomBytes }: { nowMs: number; randomBytes?: RandomBytes },
  action: (ctx: WriteContext) => Promise<T>,
): Promise<T> {
  return db.writeTransaction(async (tx) =>
    action({ tx, state: await requireDeviceState(tx), nowMs, randomBytes }),
  );
}
...
async function openSession(
  { tx, state, nowMs, randomBytes }: WriteContext,
  practiceId: string,
  steps: number,
): Promise<Session> {
...
async function recordEvent(
  ...
  const event: CountEvent = {
    id: uuidv7({ nowMs: ctx.nowMs, randomBytes: ctx.randomBytes }),
